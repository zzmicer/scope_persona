...
- [ ] Phase 3c follow-ups (NOT done): TE path is now selectable + no longer crashes but is STILL render-unverified on Blackwell (needs `transformer-engine` install + a pod run). 2-step S2 schedule (`[1000, 681]`) is still a heuristic subsample, not a confirmed distilled schedule. Generator checkpoint is still read at wrapper construction AND again in `setup_nvfp4_pipeline` (correct file now, but redundant — a `defer_weight_load` flag on the wan2_2 wrapper would remove the double read).
