...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # cross-attention
        x = x + self.cross_attn(
            self.norm3(x), context, None, crossattn_cache=crossattn_cache
        )
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _inner_forward(
        self, x, e0, freqs_cos, freqs_sin, context, cache_ks, cache_vs, crossattn_caches
    ):
        """Pure-tensor inner forward through all blocks."""
        new_ks = []
        new_vs = []
        for block_index, block in enumerate(self.blocks):
            crossattn_cache = (
                crossattn_caches[block_index] if crossattn_caches is not None else None
            )
            x, (new_k, new_v) = block(
                x,
                e=e0,
    ...
    def _forward_inference(
        ...
        **kwargs,
        ...
        Args:
            ...
            seq_len (int, optional): Unused, kept for API compat with generator wrapper
            kv_cache (list): List of cache dicts (one per block), each with 'k' and 'v'
            crossattn_cache (list): List of crossattn cache dicts (one per block)
        ...
        x, new_kvs = self._inner_forward(
            x, e0, freqs_cos, freqs_sin, context, cache_ks, cache_vs, crossattn_cache
        )
